...
unattended once claimed.
  ...
  uninstall  take out what setup put here for one installation (-home dir):
             ...
             -binary also removes the installation's own binary (an npm
             copy is npm's to remove). -purge-state also destroys the
             participant state after you type the wallet address at a
             terminal; -yes never answers it. -dry-run changes nothing
